...
public class Glavnai extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Cursor cursor3 = database3.query(PrepodDBContract.PrepodEntry.TABLE_NAME, null, null, null, null, null, null);
        while (cursor3.moveToNext()) {
            int id = cursor3.getInt(cursor3.getColumnIndexOrThrow(PrepodDBContract.PrepodEntry._ID));
            if (global.FIO == id) {
                String prfam = cursor3.getString(cursor3.getColumnIndexOrThrow(PrepodDBContract.PrepodEntry.COLUMN_FAMILIA));
                String prima = cursor3.getString(cursor3.getColumnIndexOrThrow(PrepodDBContract.PrepodEntry.COLUMN_IMA));
                String protch = cursor3.getString(cursor3.getColumnIndexOrThrow(PrepodDBContract.PrepodEntry.COLUMN_OTCHESTVO));
                String fio=prfam +" "+prima.charAt(0)+"."+protch.charAt(0)+".";
                FIO_prepoda=fio;
                this.setTitle(fio);
            }
        }
        cursor3.close();
        this.setTitle(FIO_prepoda+" "+den_n);
        getPara();
        getData(den_n);
    }
    ...
    public void getData(String Den_nedeli){
        ...
        while (cursor.moveToNext())
        {
            ...
            int id_kab = cursor.getInt(cursor.getColumnIndexOrThrow(RaspisanieDBContract.RaspisanieEntry.COLUMN_ID_KABINETA));
            ...
            {
                int id_kabineta=cursor6.getInt(cursor6.getColumnIndexOrThrow(KabinetDBContract.KabinetEntry._ID));
                ...
                if(id_kabineta==id_kab){
                    namekab=nomer_kabinet;
                }
            }
            cursor6.close();
            ...
            if(den_nedeli.toString().trim().equalsIgnoreCase(Den_nedeli)){
                ...
                while (cursor1.moveToNext())
                {
                    ...
                    {
                        int id_gruppi = cursor2.getInt(cursor2.getColumnIndexOrThrow(GruppaDBContract.GruppaEntry._ID));
                        if(idgr==id_gruppi){
                    ...
                    if(id_predmeta_gruppi==id_pg){
                        ...
                        while (cursor5.moveToNext())
                        {
                            ...
                            if(idppp==idpredpoprep){
                                ...
                                cursor4.close();
                                String gr_predmet_kab=namegr+" "+predme+" "+namekab;
                                //фио, предмет, день недели
                                if(FIO_prepoda.trim().equalsIgnoreCase(fio)){
                                    if(nomer_pari==1){
                                        if(vid_nedeli.trim().equalsIgnoreCase("Зеленая")){
                                            Para1_ZN.setText(gr_predmet_kab);
                                        }
                                        else if(vid_nedeli.trim().equalsIgnoreCase("Красная")){
                                            Para1_KN.setText(gr_predmet_kab);
                                        }
                                    }
                                    else if(nomer_pari==2){
                                        if(vid_nedeli.trim().equalsIgnoreCase("Зеленая")){
                                            Para2_ZN.setText(gr_predmet_kab);
                                        }
                                        else if(vid_nedeli.trim().equalsIgnoreCase("Красная")){
                                            Para2_KN.setText(gr_predmet_kab);
                                        }
                                    }
                                    else if(nomer_pari==3){
                                        if(vid_nedeli.trim().equalsIgnoreCase("Зеленая")){
                                            Para3_ZN.setText(gr_predmet_kab);
                                        }
                                        else if(vid_nedeli.trim().equalsIgnoreCase("Красная")){
                                            Para3_KN.setText(gr_predmet_kab);
                                        }
                                    }
                                    else if(nomer_pari==4){
                                        if(vid_nedeli.trim().equalsIgnoreCase("Зеленая")){
                                            Para4_ZN.setText(gr_predmet_kab);
                                        }
                                        else if(vid_nedeli.trim().equalsIgnoreCase("Красная")){
                                            Para4_KN.setText(gr_predmet_kab);
                                        }
                                    }
                                    else if(nomer_pari==5){
                                        if(vid_nedeli.trim().equalsIgnoreCase("Зеленая")){
                                            Para5_ZN.setText(gr_predmet_kab);
                                        }
                                        else if(vid_nedeli.trim().equalsIgnoreCase("Красная")){
                                            Para5_KN.setText(gr_predmet_kab);
                                        }
                                    }
                                    else if(nomer_pari==6){
                                        if(vid_nedeli.trim().equalsIgnoreCase("Зеленая")){
                                            Para6_ZN.setText(gr_predmet_kab);
                                        }
                                        else if(vid_nedeli.trim().equalsIgnoreCase("Красная")){
                                            Para6_KN.setText(gr_predmet_kab);
                                        }
                                    }
                                }
                            }
